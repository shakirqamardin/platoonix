...
def compute_loader_platform_fee_gbp(amount_gbp: float, settings: Settings) -> tuple[float, str]:
    """
    Loader platform fee = max(minimum, load_value × percent).
    ...
    fee = round(max(minimum, pct_part), 2)
...
def compute_job_payment_splits(amount_gbp: float, settings: Settings) -> JobPaymentSplits:
    ...
    - Loader pays amount_gbp + flat_fee_gbp where flat_fee_gbp = max(min £, percent of load).
    """
    amt = max(0.0, float(amount_gbp or 0.0))
    ...
    flat_fee_gbp, loader_fee_detail = compute_loader_platform_fee_gbp(amt, settings)
